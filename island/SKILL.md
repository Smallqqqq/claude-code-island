...
| `src/hosts/macos/` | macOS 原生主机 Swift 源码 |
...
| **Xcode CLI Tools** | macOS：编译 Swift 原生窗口 | `xcode-select --install` |
...
macOS:
```bash
test -f ~/.claude/skills/island/src/island-host-bin
swiftc --version
```
...
### 6. /island notch <mode> — 刘海模式 (仅 macOS)

可选值: `auto`（自动检测）、`normal`（强制关闭）、`notch`（强制开启）
...
1. 查看日志：
   ...
   macOS: `tail -20 ~/.claude/claude-island.log`
