...
import { getScreenGeometry, getScreenCount, computeWindowPosition, resolveNotchMode } from "./platform.mjs";
...
try {
  if (process.platform === "win32") {
    ...
    execSync("pkill -f island-host-bin", { timeout: 3000, stdio: "pipe" });
  }
...
log("info", `screenPref=${SCREEN_PREF} notchPref=${NOTCH_PREF} theme=${THEME_PREF}`);
...
const autoMode = resolveNotchMode(NOTCH_PREF, screenGeos[0].notch);
log("info", `notchMode=${autoMode} windows=${screenGeos.length}`);
...
function initWindow(w) {
  ...
  w.send('window.island.setMode(' + JSON.stringify(autoMode) + ')');
