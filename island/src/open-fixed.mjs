...
// macOS  → island-host-bin  (Swift + WKWebView, compiled from hosts/macos/)
// Windows → hosts/windows/island-host-win.exe  (C# WinForms + WebView2)
// Both speak the same stdin/stdout JSON-line protocol: html / eval / close.
...
function resolveBinary() {
  ...
  if (process.platform === "darwin") {
    ...
    throw new Error(
      "claude-island: macOS host binary not found.\n" +
      "Run: node " + join(here, "build.mjs") + "  (requires Xcode CLI Tools: xcode-select --install)"
    );
  }
  ...
  throw new Error("claude-island: unsupported platform " + process.platform);
}
